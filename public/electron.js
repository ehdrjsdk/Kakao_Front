...
function configureSession() {
    ...
    electron_1.session.defaultSession.webRequest.onHeadersReceived(filter, function (details, callback) {
        var cookies = (details.responseHeaders['set-cookie'] || []).map(function (cookie) { return cookie.replace('SameSite=Lax', 'SameSite=None'); });
        if (cookies.length > 0) {
